...
def index(request):
    ...
    next_live_date = timezone.now() + timedelta(hours=1)
# Récupérer le dernier LiveStream
    try:
        live_stream = LiveStream.objects.latest('created_at')
        next_live_date = live_stream.start_time
    except LiveStream.DoesNotExist:
        live_stream = None
        next_live_date = timezone.now() + timedelta(hours=1)
        # Imprimer chaque LiveStream dans la console
